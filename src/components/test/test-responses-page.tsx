...
const TestResponsesPage: React.FC<TestResponsesPageProps> = ({ test: testProp }) => {
  ...
  const loadResponses = async () => {
    ...
    try {
      ...
      if (!testId) return;
      const response = await getByTestId(testId, currentPage, pageSize);
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead className="uppercase tracking-wider border-b-2">
              <tr>
                <th className="px-3 py-2 w-[45%]">Usuario</th>
                <th className="px-3 py-2 w-[15%] text-center">Score</th>
                <th className="px-3 py-2 w-[15%] text-center">Pts</th>
                <th className="px-3 py-2 w-[25%] text-center">Fecha</th>
              </tr>
            </thead>
            <tbody>
              {data.length === 0 && (
