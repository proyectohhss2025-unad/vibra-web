import { PreTestResponse } from '@/models/preTest.entity';
import { DocumentDuplicateIcon, PencilIcon, StarIcon, StopIcon } from '@heroicons/react/solid';
import React from 'react';
import './preTest.css';
import { getSafeKeyFromStorage } from '@/utils/safe-token-storage';

interface Action {
    name: string;
    color: string;
    handler: (preTest: PreTestResponse) => void;
}

interface TableProps {
    data: PreTestResponse[];
    actions: Action[];
    children?: React.ReactNode;
}

const Table: React.FC<TableProps> = ({ data, actions, children }) => {
    return (
        <div id='preTest' className="relative overflow-x-auto mt-0" style={{ marginTop: '-20px' }}>
            <table className="rounded-lg min-w-full text-left text-sm">
                <thead className="uppercase tracking-wider border-b-2">
                    <tr>
                        {children}
                        {actions.map((action) => (
                            <th className="text-left px-3 py-1" key={action.name}>{getSafeKeyFromStorage(action.name)}</th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                    {data.map((record) => (
                        <tr className="mt-0 mb-0 hover:bg-blue-100 hover:rounded border-b" key={record._id}>
                            <td className="text-sm px-2 py-1.5 font-mono text-xs" style={{width:"200px"}} >{record.testId}</td>
                            <td className="text-sm px-2 py-1.5">{record.userId}</td>
                            <td className="text-sm px-2 py-1.5">{record.responses?.length ?? 0}</td>
                            <td className="text-sm px-2 py-1.5">
                                <span className="px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    {record.totalScore}
                                </span>
                            </td>
                            {actions.map((action) => (
                                <td className="text-center px-2 py-1.5 text-sm" key={action.name}>
                                    <button
                                        className="action-blue inline-block px-1 py-1 text-white rounded-md"
                                        onClick={() => action.handler(record)}>
                                        {action.name === 'CopyID' && <DocumentDuplicateIcon name="copyID" className="h-5 w-5 text-indigo-600" color={action.color} data-tooltip-id="my-tooltip-p" data-tooltip-content="Click to copy ID" />}
                                        {action.name === 'Edit' && <PencilIcon name="success" className="h-5 w-5 text-blue-600" color={action.color} />}
                                    </button>
                                </td>
                            ))}
                        </tr>
                    ))}
                </tbody>
            </table>
        </div>
    );
};

export default Table;
